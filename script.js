...
function renderSidebar(list) {
    document.getElementById('patient-list').innerHTML = list.map(p => `
        <div class="patient-item ${p.name === 'Jessica Taylor' ? 'active' : ''}" data-name="${p.name}">
            ...
            <img src="more_horiz.svg" style="width:18px;">
        </div>
    `).join('');
}
...
function renderPatient(p) {
    ...
    document.getElementById('resp-val').innerText =
        latest.respiratory_rate.value + ' bpm';

    document.getElementById('temp-val').innerText =
        latest.temperature.value + '°F';

    document.getElementById('heart-val').innerText =
        latest.heart_rate.value + ' bpm';

    document.getElementById('sys-val').innerText =
        latest.blood_pressure.systolic.value;

    document.getElementById('dia-val').innerText =
    ...
    document.getElementById('lab-list').innerHTML =
        p.lab_results.map(l =>
            `<li class="lab-item">
                <span>${l}</span>
                <img src="download.svg" style="width:18px;">
            </li>`
        ).join('');
